...
'''
    支持的数据类型：
        value type: uint32, sint32, uint64, sint64, bool, string (其中uint32, sint32, string可以用作key列)
...
class Sheet:
    ...
    def __GetValueType(self, col_num):
        ...
        if self.__IsValueType(value_type_str):
            ...
            LogHelp.LogError('配置表：{} {}行，{}列， 期望:值类型， 实际: {}'.format(ParsingEnv.GetCurSheetPath(),  col_num,3, value_type_str))
    ...
    def __CreateOptional(self, col_num, export_mask):
        ...
        if ConfigMgr.Client_CKey_Tag in self.first_line[col_num] or ConfigMgr.Client_CSKey_Tag in self.first_line[col_num]:
    ...
    def __PopActiveStruct(self, name):
        ...
        if False and ExportToClient(pre_struct.export_mask):
            ...
            if has_attri and client_attri_count  == 0:
                LogHelp.LogError("配置表： {}  {}在生成客户端数据时为空结构体(结构体为C或CS，但是所有结构体内属性都为S)!".format(ParsingEnv.GetCurSheetPath(), name))
    ...
    def __CreateRepeated(self, col_num, export_mask):
        ...
        if self.__IsValueType(flag):
            ...
            try:
                ...
                LogHelp.LogError('配置表：{} {}列，{}行， 期望:整数  实际: {}'.format(ParsingEnv.GetCurSheetPath(),  col_num + 1,4, flag))
            ...
            try:
                ...
                LogHelp.LogError('配置表：{} {}列，{}行， 期望:整数  实际: {}'.format(ParsingEnv.GetCurSheetPath(),  col_num + 2,3, self.third_line[col_num + 1].strip()))
    ...
    def __CreateAttribute(self, col_num, export_mask):
        ...
        if qualifer == ConfigMgr.Optional_Qualifer:
            ...
            LogHelp.LogError('配置表：{} {} 行， {}列， 期望为：required或optional或repeated, 实际为: {}'.format(ParsingEnv.GetCurSheetPath(),  col_num + 1,2, qualifer))
