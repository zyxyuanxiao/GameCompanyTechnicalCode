...
path = os.getcwd() + "/Windows/py3-Tools/Ini/"
sys.path.append(path)
...
match_getConfigDataByTwoID = re.compile( r'getConfigDataByTwoID\(' + r_match_key + r',' + r_match_key_with_value + r_match_key_with_value + r'\)')
match_getConfigDatasByTwoID = re.compile( r'getConfigDatasByTwoID\(' + r_match_key + r',' + r_match_key_with_value + r_match_key_with_value + r'\)')
match_getConfigDataByThreeID = re.compile(r'getConfigDataByThreeID\(' + r_match_key + r',' + r_match_key_with_value + r_match_key_with_value + r_match_key_with_value + r'\)')
...
class ParseClientCode:
    ...
    def GenLog(self):
        with open("Windows/Log/conflict_key_info_output.txt", 'w+') as file:
            for config_name, info in self.config_info.items():
                if config_name in self.conflict_config_info:
                    file.write(config_name + '\n')
                    file.write(str(info))

        with open("Windows/Log/parse_csv_key_info_output.txt", 'w+') as file:
            for config_name, info in self.config_info.items():
                if config_name in self.conflict_config_info:
                    continue
                file.write(config_name + '\n')
    ...
    def GenNotUsedByClientLog(self):
        ...
        with open("Windows/Log/not_used_by_client.txt", 'w+') as f:
            for path in final_p:
