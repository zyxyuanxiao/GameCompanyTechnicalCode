...
# @author: jameyli <lgy AT live DOT com>(yuexianliu修改)
# @brief:  xls 配置导表工具
...
# 但是目前只支持整形
# 2019-4-17 添加功能
# repeated 后的嵌入可以读取处理中文
# 2019-4-29 修改读取文件方式
# 应要求以后使用csv，不再使用excel，修改了读取文件方式
...
class SheetInterpreter:
    ...
    def __init__(self, xls_file_path, sheet_name, op2):
        ...
        try :
            with open(self._xls_file_path,'r') as csvfile:
                headCount = 0
                reader = csv.reader(csvfile)
                for row in reader:
                    self.csvArrays.append(row)
                    headCount += 1
                    if headCount > FIELD_COMMENT_ROW:
                        break
        except BaseException, e :
            print "open csv file(%s) failed!"%(self._xls_file_path)
            raise
        
        # 行数和列数
        self._row_count = len(self.csvArrays)
        self._col_count = len(self.csvArrays[0])
    ...
    def _FieldDefine(self, repeated_num) :
        ...
        belong_rule = str(self.csvArrays[FILED_CS_BELONG_ROW][self._col]).lower()
        field_rule = str(self.csvArrays[FIELD_RULE_ROW][self._col])  
        ...
        if field_rule == "required" or field_rule == "optional" :
            field_type = str(self.csvArrays[FIELD_TYPE_ROW][self._col]).strip()
            field_name = str(self.csvArrays[FIELD_NAME_ROW][self._col]).strip()
            field_comment = unicode(self.csvArrays[FIELD_COMMENT_ROW][self._col])
            ...
            second_row = str(self.csvArrays[FIELD_TYPE_ROW][self._col]).strip()
            ...
            field_num = int(float(self.csvArrays[FIELD_TYPE_ROW][self._col]))
...
class DataParser:
    ...
    def Parse(self) :
        ...
        for id_col in range(0, self._col_count) :
            info_id = str(self.csvArrays[self._row][id_col]).strip()
            if info_id == "" :
                continue
            else :
                break
        ...
        for self._row in range(5, self._row_count) :
            ...
            #info_id = str(self._sheet.cell_value(self._row, id_col)).strip()
             ...
             #策划需要的在某个行进行注释
            if "//" in str(self.csvArrays[self._row][id_col]).strip():
                continue
        ...
        # Roshan:部分表格进行Id升序排序，减少查找时的消耗
        if self._sheet_name == "MonsterProperty":  # 怪物属性表
            print("Add by Roshan : MonsterProperty sort by ID !!!!!!!!!!!!")
            item_array.items.sort(key=lambda MonsterProperty: MonsterProperty.id, reverse=False)
        elif self._sheet_name == "GoodConfig":  # 物品道具表
            print("Add by Roshan : GoodConfig sort by ID !!!!!!!!!!!!")
            item_array.items.sort(key=lambda GoodConfig: GoodConfig.ID, reverse=False)
    ...
    def _ParseField(self, max_repeated_num, repeated_num, item) :
        ...
        field_rule = str(self.csvArrays[FIELD_RULE_ROW][self._col]).strip()
        ...
        if field_rule == "required" or field_rule == "optional" :
            ...
            if second_row.isdigit() or second_row.find(".") != -1 :
                ...
                read = self.csvArrays[self._row][self._col]
                repeated_num = 0 if read == "" else int(float(self.csvArrays[self._row][self._col]))
                ...
                field_type = second_row
                field_name = str(self.csvArrays[FIELD_NAME_ROW][self._col]).strip()
                field_value_str = unicode(self.csvArrays[self._row][self._col])
    ...
    def _GetFieldValue(self, field_type, row, col) :
        ...
        field_value = self.csvArrays[row][col]
            ...
            if field_type == "int32" or field_type == "int64"\
                    ...
                    or field_type == "sfixed32" or field_type == "sfixed64" :
                        if len(str(field_value).strip()) <=0 :
                            ...
                            #  差异表支持 Add by yuexianliu
                            temp = GlobalUtilValue.DoGlobalTranslate(field_value,self.csvArrays, row, col, self._sheet_name ,localConfig)
                            if temp is not None and len(temp) > 0:
                                field_value = temp

                            #增加无符号的类型，填成了负数的检测 lynn
                            data = int(float(field_value))
                            if field_type == "uint32" or field_type == "uint64":
                                if data < 0:
                                    self._PrintBadField("Unsigned Num Is Less 0")
    ...
    def _PrintBadField(self, strReason):
        strFieldName = str(self.csvArrays[FIELD_NAME_ROW][self._col]).strip()
...
if __name__ == '__main__' :
    ...
    #获取语言
    localConfig = sys.argv[4]
    localConfig = localConfig.lower()
    ...
    if op == 0 or op == 2:
        GlobalUtilValue.bestDic = GlobalUtilValue.ReadBestFile(sheet_name,localConfig)
        try :
