...
def GenSingleConfig(config_name, full_path, gen_bin, gen_csharp, gen_lua, gen_proto, gen_proto_bin):
    ...
    if gen_proto:
        try:
            with open(ConfigMgr.Proto_Output_Dir + 'dataconfig_{}.proto'.format(config_name.lower()), 'w+', encoding='utf-8') as proto_file:
    ...
    if gen_proto_bin:
        try:
            with open(ConfigMgr.Proto_Bin_Output_Dir + 'dataconfig_{}.data'.format(config_name.lower()), 'wb+') as pb_bin_file:
...
# 将修改好的 shell 替换其他文件夹里面的 shell 再设置名字
def ChangeAllBat():
    normalShell = "C:/TW/BuildDataConfig/Windows/Windows-bat/BuildSingle.bat"
    ...
    for root, dirs, files in list_dirs:
        ...
        for name in files:
            if ".bat" in name:
                ...
                os.remove(path)
            path = root + "/" + "BuildSingle.bat"
            path = str(path).replace("//","/")
            shutil.copyfile(normalShell, root + "/" + "BuildSingle.bat")


if __name__ == "__main__":
    # messagebox.showerror("错误", "打表错误,请查找终端提示内容")
    ChangeAllShell()
    # ChangeAllBat()
