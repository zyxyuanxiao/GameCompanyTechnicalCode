...
extern "C"
{
	...
	extern float ReadFloat(byte* data, int offset) {
		float number = ReadNumber<std::int64_t>(data, offset);
		number = number/4294967295;//根据定点数中的 浮点数转换规律定制的
        return number;
    }
	...
	extern const char * ReadString(byte* data, int offset, int &arraySize) {
		...
		size_t len = strlen(str) > BUFF_LEN ? BUFF_LEN : strlen(str);
		arraySize = (int)len;
